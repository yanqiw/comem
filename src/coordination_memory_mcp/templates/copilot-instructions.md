...
This repository uses the coordination-memory MCP server. Follow the
**Coordination Memory protocol** in `AGENTS.md` (the section between the
`coordination-memory` markers).

- `completed_gate_passed` is not accepted truth; only an integrator
  `accept_event` makes work accepted.
- Claim assignments before working; pass the current `base_revision` on mutating
  calls.
